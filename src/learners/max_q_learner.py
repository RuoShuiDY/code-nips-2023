...
from utils.rl_utils import build_td_lambda_targets_with_value
import torch as th
...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args, ban):
        self.args = args
        self.mac = mac
        self.logger = logger
    ...
    def train_with_template(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if not self.tree.warm_up:
            relay, pos = self.tree.judge_templates(states, rewards)
        else:
            relay, pos = th.zeros(states.size(0), self.tree.max_seq_length), None
        ...
        if self.args.use_bandit:
            td_lambda = self.ban.sample()
        else:
            td_lambda = self.args.td_lambda                   
        targets = build_td_lambda_targets_with_value(rewards, terminated, mask, target_max_qvals, self.args.n_agents, self.args.gamma, td_lambda, relay, pos)

        # Td-error
        td_error = (chosen_action_qvals - (targets.detach()[:, :-1]))
        ...
        central_td_error = (central_chosen_action_qvals - targets.detach()[:, :-1])
        ...
        if self.args.hysteretic_qmix: # OW-QMIX
            ...
            qtot_larger = targets[:, :-1] > max_action_qtot
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            relay = relay.sum(dim=-1).cpu().tolist()
            
            bandit_info = {'arm_{}'.format(i):p for i, p in enumerate(self.ban.get_probs())}
            bandit_info.update({'relay_5': np.percentile(relay, 5), 'relay_95': np.percentile(relay, 95), 'loss': loss.item(), 'timestep': t_env})
